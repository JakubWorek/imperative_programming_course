...
int top=-1;   // jawne zerowanie nie jest konieczne
...
double queue_push(int how_many) {
    int cp_how_many=how_many;

    if(cp_how_many>QUEUE_SIZE - in){
        cp_how_many = QUEUE_SIZE - in;
    }

    for(int i=0; i<cp_how_many; i++){
        curr_nr++;
        queue[in] = curr_nr;
        in++;
    }

    if(cp_how_many != how_many){
        while(cp_how_many < how_many){
            curr_nr++;
            cp_how_many++;
        }
        return INFINITY;
...
int queue_pop(int how_many) {
    ...
    for(int i=0; i<QUEUE_SIZE-how_many; i++){
