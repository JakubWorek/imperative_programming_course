...
void aver_varian(const double tab[], size_t n, double *arith_average, double *variance) { // działa
    for(int i = 0; i < n; i++)
        *arith_average += tab[i];

    *arith_average = *arith_average / n;
...
// 1 - sukces; 0 - porażka
void bernoulli_gen(int tab[], size_t n, double probab) { //działa
    double liczba = 0.0;
...
void two_dice_probab_distrib(double distrib[], int throws_num) { //działa
    int rzut1 = 0, rzut2 = 0;
...
void cum_discret_distrib(double distrib[], size_t n) { // działa
    for(int i=1; i<n; i++){
...
void monty_hall(int *p_switch_wins, int *p_nonswitch_wins, int n) { // działa
    *p_switch_wins = 0;
    *p_nonswitch_wins = 0;

    for (int i = 0; i < n; i++) {
        int prize_door = rand() % 3;
        int first_choice = rand() % 3;


        // Sprawdzenie, czy gracz wygrał po zmianie decyzji
