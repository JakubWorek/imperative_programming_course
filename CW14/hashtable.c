...
typedef data_union (*CreateDataFp)();
...
void rehash(hash_table *p_table) {
    ...
    while(p){
        size_t i = p_table->hash_function(p->data, p_table->size);
        q = p->next;
        p->next = p_table->ht[i];
        p_table->ht[i] = p;
        p = q;
...
//		p_table->create_data(&data);
        data = p_table->create_data(); // should give the same result as the line above
        switch (op) {
            case 'r':
